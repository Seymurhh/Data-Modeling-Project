...
ggplot(df_house, aes(x = factor(grade), y = price)) + 
  geom_boxplot() +
  labs(title = "Boxplot of Prices by House Grade", 
       x = "Grade", y = "Price")
...
```
**Analysis**:
For Boxcox transormation, the optimal lambda is -0.1 with 95% confidence interval range which is close to zero. Although, we can estimate lambda to be zero and apply logarithmic transformation of the data, we have done the boxcox transformation with exact optimal lambda value for better accuracy.

```{r}

bc <- boxcox(house_lm, lambda=seq(-10, 10, by=1))
...
house_lm1<-lm(price^optimal_lambda~.,data=train.dat)
summary(house_lm1)
par(mfrow=c(2,2))
plot(house_lm1)
...
# Test data Predictions
pred<- predict(house_lm1,test.dat)^(1/optimal_lambda)
act<-test.dat$price
...
plot(stepwise_model$rsquare, 
     xlab=NA, ylab="R-Square", pch=20, col="blue", type="b")
plot(stepwise_model$aic, 
     xlab=NA, ylab="AIC", pch=20, col="blue", type="b")
plot(stepwise_model$sbc, 
     xlab=NA, ylab="SBC", pch=20, col="blue", type="b")
plot(stepwise_model$adjr, 
     xlab=NA, ylab="Adj. R-Square", pch=20, col="blue", type="b")
plot(stepwise_model$mallows_cp, 
     xlab=NA, ylab="Mallows Cp", pch=20, col="blue", type="b")
plot(stepwise_model$sbic, 
     xlab=NA, ylab="SBIC", pch=20, col="blue", type="b")
...
plot(depth_values, mse_values, type = "b", 
     xlab = "Depth Value", ylab = "Root Mean Squared Error", 
     main = "Regression Tree Cross Validation (RMSE vs depth)")
...
plot(depth_values, test_rsq_values, type="o", 
     xlab = "Depth Value", ylab = "Test R-squared", 
     main = "Regression Tree Cross Validation (R2 vs depth)")
text(depth_values, test_rsq_values, 
     labels=round(test_rsq_values,4), pos=3, xpd=TRUE)
...
# fix permuation importance - 
# better for mixed data (ordinal, categorical, quantitative, etc)
...
hist(svm_errors, 
     main="SVM Prediction Error Distribution", 
...
Supplementary Analyses and Visuals
