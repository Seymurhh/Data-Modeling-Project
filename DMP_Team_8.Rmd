...
house_lm__test_pred = predict(house_lm, newdata = test.dat)
# Test MSE
house_lm_test_mse = mean((house_lm_pred-test.dat$price)^2)
# Test RMSE
house_lm__test_Rmse = sqrt(house_lm_mse)
# Test Residuals
house_lm__test_residuals = test.dat$price-house_lm_pred
# Test R-squared
house_lm_test_rsq = 1- var(house_lm_residuals)/var(test.dat$price)
# SSE
house_lm_test_sse_test <- sum((test.dat$price - house_lm__test_pred)^2)
...
results.df = data.frame(model= "Linear Regression All (Baseline)",
                 ...
                 RMSE.test = house_lm__test_Rmse,
                 SSE.test = house_lm_test_sse_test)
```
...
#### \color{blue} *Analysis*: The ROC curve demonstrates perfect classification performance with maximal sensitivity and specificity
...
#### \color{blue} *Analysis*: The Variable Importance plot from a Random Forest model highlights 'price.binary' as the most influential predictor, with 'sqft_living', 'grade', and 'sqft_living15' also contributing significantly to model accuracy, suggesting their strong predictive power in the Random Forest model.
