...
# Predictions for each model, excluding stepwise_model
predictions <- data.frame(
  price = train.dat$price,
  price.lm = predict(house_lm1, train.dat),
  # price.sw.lm = predict(stepwise_model, train.dat), # Excluded due to predict method issue
  price.ridge = predict(house_lm_ridge, s = best.lambda.ridge, newx = data.matrix(train.dat[price.predictors])),
  price.lasso = predict(house_lm_lasso, s = best.lambda.lasso, newx = data.matrix(train.dat[price.predictors])),
  price.en = predict(house_lm_enet, s = best.lambda.enet, newx = data.matrix(train.dat[price.predictors])),
  price.huber = predict(house_lm_huber, train.dat)
)
...
calc_metrics <- function(actual, predicted) {
  ...
  r2 <- 1 - sse / sst
  return(c(SSE = sse, MSE = mse, RMSE = rmse, R2 = r2))
}

# function to each set of predictions
metrics <- data.frame(
  Model = c("Linear", "Ridge", "Lasso", "Elastic Net", "Huber"),
