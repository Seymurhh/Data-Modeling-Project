...
metrics <- data.frame(
  ...
  Model = c("Linear", "Stepwise", "Weighted Least Squares", "Huber", "Bisquare"),
...
# Remove redundant, unnecessary columns from dataset. 
#df_house_nn <- df_house_original
#df_house_nn[c("date","zipcode", "day")] <- list(NULL)

#set.seed(1023)
#n<-dim(df_house_nn)[1]
#IND<-sample(c(1:n),round(n*0.7))
#train.dat.nn<-df_house_nn[IND,]
#test.dat.nn<-df_house_nn[-c(IND),]
...
house_NN <- neuralnet(price ~ ., data = train.dat.norm, hidden = 5, linear.output = T)
...
>>>>>>> 83ec1524afeeeb6444274e4813b3eb9439001a97
\newpage
