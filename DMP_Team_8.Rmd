...
```{r}
# Ibrahim
# Stepwise both ways selection method

# stepboth_model <- ols_step_both_p(house_lm1, train.dat)
stepwise_model <- step(house_lm1, train.dat)
stepwise_model
par(mfrow=c(2,2))
plot(stepwise_model)
...
```{r}
# Plotting the main (boxcox) model that used the train data
par(mfrow=c(2,2))
plot(house_lm1)
```
...
vif(house_lm1)
```
yr_built, yr_rennovated, and renovated indicate multicollinearity issues.

```{r}
# Testing the weighted-least squares model for multicollinearity
vif(house_lm_wls)
```
...
train.dat$price.lm <- stats::predict(house_lm1, train.dat)
train.dat$price.sw.lm <- stats::predict(stepwise_model, train.dat)
train.dat$price.ridge <-
  stats::predict(house_lm_ridge, s = best.lambda.ridge, 
                 newx = data.matrix(train.dat[price.predictors]))[,1]
train.dat$price.lasso <-
  stats::predict(house_lm_lasso, s = best.lambda.lasso, 
                 newx = data.matrix(train.dat[price.predictors]))[,1]
train.dat$price.en <-
  stats::predict(house_lm_enet, s = best.lambda.enet, 
                 newx = data.matrix(train.dat[price.predictors]))[,1]
train.dat$price.huber <- stats::predict(house_lm_huber, train.dat)

# SST, SSE, and R2
sst <- sum((train.dat$price - mean(train.dat$price))^2)
sse.lm <- sum((train.dat$price - train.dat$price.lm)^2)
sse.sw <- sum((train.dat$price - train.dat$price.sw.lm)^2)
sse.ridge <- sum((train.dat$price - train.dat$price.ridge)^2)
sse.lasso <- sum((train.dat$price - train.dat$price.lasso)^2)
sse.enet <- sum((train.dat$price - train.dat$price.en)^2)
sse.huber <- sum((train.dat$price - train.dat$price.huber)^2)
data.frame(Model = c('Linear', 'Stepwise', 'Ridge', 'Lasso', 'Elastic Net', 'Huber'),
           ...
           MSE = c(round(sse.lm / nrow(train.dat), digits = 4),
                  round(sse.sw / nrow(train.dat), digits = 4),
                  round(sse.ridge / nrow(train.dat), digits = 4),
                  round(sse.lasso / nrow(train.dat), digits = 4),
                  round(sse.enet / nrow(train.dat), digits = 4),
                  round(sse.huber / nrow(train.dat), digits = 4))) %>%
