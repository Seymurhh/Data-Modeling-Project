...
In this step, we meticulously construct a linear regression model, leveraging the training dataset to ascertain how well our predictors explain the variability in house prices. The initial model fitting is a critical juncture where we assess the significance of each predictor and the overall strength of the model. [ @faraway2014linear ]
...
results.df <- data.frame(model = "Linear Regression Model - Original",
...
results.df = rbind(results.df, data.frame(
  model = "Linear Regression without Insignificant Predictors",
  R.Squared.Train = summary(house_lm)$r.square,
  R.Squared.Test = house_lm_test_rsq,
  RMSE.test = house_lm_test_rmse,
  SSE.test = house_lm_test_sse))
...
results.df = rbind(results.df, 
                   data.frame(model = "Linear Regression after BoxCox",
...
**Analysis:** The output indicates an improvement in the model's performance on the test data after applying the Box-Cox transformation. The $R^2$ value for the training dataset had a significant increase of almost 8%, while the $R^2$ for the test data increased minimally from 0.8123883 to 0.8172112, indicating the same fit to the test data. Similarly, the RMSE decreased minimally as well, suggesting more accurate predictions from the transformed model. The SSE also decreased, further confirming the enhanced predictive performance post-transformation. Overall, the Box-Cox transformation has led to a model that explains a greater proportion of variance in house prices and predicts them with higher accuracy, as evidenced by the improved R-squared and lower RMSE and SSE values. This demonstrates the effectiveness of the transformation in stabilizing variance and improving model fit for the data at hand.


\newpage
# IV. Model Performance Testing

This Section is dedicated to rigorously assessing the predictive capabilities of our various regression models. This section utilizes the test dataset as a proving ground to evaluate how well each model generalizes beyond the data on which it was trained. We employ a methodical approach, beginning with the stepwise selection process to fine-tune the models, followed by the application of BoxCox transformations to meet the linearity assumption where necessary. The section scrutinizes the assumption adherence of the final models, like linearity, homoscedasticity, normality of residuals, and independence, employing diagnostic plots and statistical tests. We also address any need for remedial measures, such as weighted least squares (WLS), ridge regression, lasso, or elastic net, to correct for issues like heteroscedasticity and multicollinearity. Each model is then compared against the others based on key performance metrics, such as R-squared values, residual standard error, and p-values from the F-statistic, to determine the champion model and its benchmark counterpart. The insights gleaned from this comprehensive testing phase are crucial for confirming the robustness of the models and ensuring their reliability in practical applications.
...
```{r Append Stepwise model to results}
...
Before performing Ridge, Lasso, and ElasticNet regression methods, we will assess the multicollinearity in out main model so far, the Linear Regression Test after BoxCox transformation.
...
```{r Append Ridge Regression Results}
...
```{r Append Lasso Regression to results}
# Append to results.df
...
```{r Append Elastic Net Regression to results}
# Append to results.df
...
**Analysis:** These are out observations from the plots:
...
```
```{r Append Robust Regression results}
...
# Append to the Results Data Frame
results.df = rbind(results.df,data.frame(model = "Robust Regression - Huber",
...
```
```{r Append Robust Regression without low weights}
...
# Append to the Results Data Frame
results.df = rbind(results.df,data.frame(model = "Robust Regression - Huber (clean)",
...
```{r Append Robust Regression Transformed and Clean}
...
# Append to the Results Data Frame
results.df = rbind(results.df,data.frame(model = "Robust Regression - Huber (transformed + clean)",
...
```
```{r Append Robust Regression using Bisquare}
...
**Analysis:** The Robust regression with the Bisquare weights has a higher residual standard error compared to the Huber weights. 


## Comparative Analysis of Model Performances

This section is dedicated to the meticulous evaluation the various statistical models developed to predict housing prices up to this point. This comparison leverages a range of performance metrics, including R-squared values for training and testing datasets, as well as the Root Mean Squared Error (RMSE) and Sum of Squares Error (SSE) on the test set. Each metric offers a unique lens through which to assess the models' accuracy, generalizability, and predictive power.

```{r}

# Evaluating the performance of all models up to this point
results.df.asc <- results.df[order(results.df$RMSE.test),]
kable(results.df.asc)

```

**Analysis:** The Linear Regression after BoxCox transformation demonstrates robust predictive capabilities, with an R-squared of 0.8833 on the training data and 0.8172 on the test data, indicating a strong ability to generalize from the training to the test set. The model's performance is mirrored closely by the Original Linear Regression and the Linear Regression without Insignificant Predictors, both showcasing R-squared values marginally over 0.80 for the test set, which suggests that the removal of insignificant predictors does not substantially detract from the model’s predictive strength.

Robust Regression models, particularly when cleaned and transformed using Huber's method, stand out with an R-squared of 0.8373 on the training set and 0.8060 on the test set, underscoring their resilience against outliers and leverage points. The Weighted Least Squared Regression, with an R-squared of 0.8929 on the training data and 0.7563 on the test data, exhibits a notable drop in performance when generalized, though it remains a strong contender.

Lasso, Elastic Net, and Ridge Regression models reflect more modest generalizability, with test R-squared scores hovering around 0.70. These regularization techniques, while mitigating overfitting, have not translated into substantial predictive improvements in this context.

The Stepwise Regression model shows a promising R-squared of 0.8833 on the training set but experiences a slight decrease to 0.7509 in the test set, which may point to some overfitting despite the method's iterative approach to feature selection.

In contrast, the Robust Regression - Bisquare model underperforms in terms of both R-squared and RMSE on the test set. Its relatively lower R-squared of 0.7406, coupled with the highest RMSE and SSE, indicates a lesser fit and predictive accuracy, particularly for outlier management.

The table provided paints a nuanced picture of model efficacy, revealing that while some models excel in training, their prowess does not always translate to unseen data. Models like the Random Forest, not shown in this specific table but discussed later, with their intricate decision-making pathways, tend to outperform simpler models, especially in complex datasets. This analysis emphasizes the importance of not only model selection but also the rigorous evaluation of model performance across multiple dimensions to ensure the selection of the most appropriate model for deployment.


## Challenges with Best Subset Regression

Our efforts to implement the Best Subset Regression faced significant computational hurdles. Despite multiple attempts, we encountered insurmountable limitations in processing power that led to exceedingly long run times and system instability, including instances where the application or the computer would become unresponsive. Even when deploying a high-performance gaming PC, the process extended beyond a three-hour mark. Given these persistent operational challenges and the disproportionate resource consumption, we made the strategic decision to exclude Best Subset Regression from our suite of analytical tools. This decision allows us to allocate our computational resources more efficiently and focus on alternative models that offer a balance between performance and practicality.
...
**Analysis:** The bar plot indicates that 'grade' has the most significant impact on the model's decisions, followed by 'sqft_living', and 'sqft_above'. Variables such as 'age', 'bedrooms', and 'yr_renovated' have minimal impact. This suggests that house quality and living area are the primary drivers of price according to the model, which aligns with real-world expectations of property valuation.
...
This subsection discusses the creation of the Support Vector Machine model [@deng2016svm]. It entails the training phase on the dataset, where the SVM algorithm learns to find the best hyperplane that categorizes the data points.
...
In this part, we assess the performance of the SVM model using the test dataset [@e1071package]. The Root Mean Square Error (RMSE) metric provides insight into the average deviation of the predicted house prices from the actual values.
...
```{r Append Neural Network results to dataframe}

# Append to the Results Data Frame
results.df = rbind(results.df,data.frame(model = "Neural Network",
            R.Squared.Train = unname(train_nn[2]),
            R.Squared.Test = unname(test_nn[2]),
            RMSE.test = unname(test_nn[1]),
            SSE.test = sse_value))
...
# Checking the correlation between predicted price and test price
model_results <- compute(house_NN, test.dat.norm[-1])
...
# Prediction Accuracy Plot for NN
...
**Analysis:** The scatter plot of actual vs. predicted prices shows a cluster spread evenly around the ideal 45-degree line, reinforcing that the model tends to predict the higher-valued houses better then others.
...
To ensure the continued efficacy of our champion model, we must:
  ...
  6. Assess the model for bias and ensure compliance with regulatory requirements, mitigating any unfair impact on specific demographic groups.
