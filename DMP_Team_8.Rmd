...
nn_train_pred = predict(house_lm_wls, newdata = train.dat.norm)
nn_test_pred = predict(house_lm_wls, newdata = test.dat.norm)

nn_train_results = postResample(pred = nn_train_pred, obs = train.dat.norm$price)
nn_test_results = postResample(pred = nn_test_pred, obs = test.dat.norm$price)
nn_test_sse = sum((nn_test_pred - test.dat.norm$price)^2)

# Append to the Results Data Frame
results.df = rbind(results.df,data.frame(model = "Neural Network",
            R.Squared.Train = unname(nn_train_results[2]),
            R.Squared.Test = unname(nn_test_results[2]),
            RMSE.test = unname(nn_test_results[1]),
            SSE.test = nn_test_sse)
)
```
