...
The Pairwise scatter plot below shows the correlation between highly correlated variables extracted from heatmap correlation matrix.
...
price.predictors.train <- colnames(dplyr::select(train.dat, -price))

ridge_results <- data.frame(
  price.train = train.dat$price,
  price.ridge.train = predict(house_lm_ridge, s = best.lambda.ridge, newx = data.matrix(train.dat[price.predictors.train]))
)
...
lasso_results <- data.frame(
  price.train = train.dat$price,
  price.lasso.train = predict(house_lm_lasso, s = best.lambda.lasso, newx = data.matrix(train.dat[price.predictors.train]))
)
...
enet_results <- data.frame(
  price.train = train.dat$price,
  price.enet.train = predict(house_lm_enet, s = best.lambda.enet, newx = data.matrix(train.dat[price.predictors.train]))
)
